...
void send_Motor_data(int16_t speed)
{
    ...
    write_object(WRITE_CMD, SPEEDLED, 0x00, 0x00, speed, chksum);
}
...
{
    /*sprintf(buff, "%f", dist);
    write_string(WRITE_STR, DISTANCE, buff, chksum);
    memset(buff, 0, 12);
    */
    write_object(WRITE_CMD, DISTANCE, 0x01, 0x00, (int)dist, chksum);
